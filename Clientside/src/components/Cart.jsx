...
const Cart = () => {
  ...
  const token = localStorage.getItem("token");
    ...
    const fetchCartItems = async () => {
      try {
        const res = await axios.get("http://localhost:3003/api/getCart", {
  ...
  useEffect(() => {
    const fetchAddresses = async () => {
      try {
        const res = await axios.get("http://localhost:3003/api/getUserAddresses", {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const incrementQuantity = async (productId) => {
    try {
      const res = await axios.put(`http://localhost:3003/api/incrementCartQuantity/${productId}`,{},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const decrementQuantity = async (productId) => {
    try {
      const res = await axios.put(
        `http://localhost:3003/api/decrementCartQuantity/${productId}`,{},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const deleteItem = async (productId) => {
    try {
      const res = await axios.delete(`http://localhost:3003/api/deleteCartItem/${productId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      const res = await axios.post("http://localhost:3003/api/placeOrder", orderData, {
        headers: { Authorization: `Bearer ${token}` },
      });
