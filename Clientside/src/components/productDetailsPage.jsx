...
const ProductDetailsPage = () => {
  ...
  useEffect(() => {
    ...
    const checkProductInCart = async () => {
      try {
        const res = await axios.get(`http://localhost:3003/api/findOnCart/${productId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
    ...
    const checkIfFavorite = async () => {
      try {
        const res = await axios.get(`http://localhost:3003/api/checkWishlist/${productId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const toggleFavorite = async () => {
    try {
      ...
      console.log(url);
      const res = await axios.post(`http://localhost:3003/api/${url}/${productId}`, {}, { headers: { Authorization: `Bearer ${token}` } });
